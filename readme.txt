=== MigWP Migrator - Migrate Your Site to MigWP ===
Contributors: migwp, ingobaab
Tags: migration, backup, migwp, site-migration, transfer
...
Move your WordPress site to MigWP's lightning-fast hosting platform with just a few clicks. No technical knowledge required!

== Description ==

Ready to give your WordPress site the speed and security it deserves? MigWP Migrator makes moving your site to MigWP's powerful hosting platform a breeze. No more complicated migration processes or downtime – just simple, stress-free transfers.

= Why Move to MigWP? =

* 🚀 **Blazing Fast Performance** - Your site will run on MigWP's optimized servers
...
3. Paste the migration key in your MigWP migrate site wizard
...
Join thousands of happy website owners who've already moved to MigWP's optimized hosting platform. Experience better performance, tighter security, and peace of mind.

== Installation ==

1. Install and activate the plugin through WordPress
2. Go to MigWP Migration in your admin menu
3. Copy your migration key
4. Enter the migration key in MigWP to start the migration

== Frequently Asked Questions ==

= Do I need technical knowledge to use this plugin? =

Not at all! If you can copy and paste your migration key, you can move your site to MigWP.
...
`GET /wp-json/migwp-migrator/v1/files/stream?path=<relative-path>&chunk=<index>&secret=<migration-key>`

Authentication:

* Use the same shared-secret mechanism as the other migration endpoints.
* Send `X-MigWP-Key: <migration-key>` or pass `secret=<migration-key>` as a query parameter.
...
* `X-MigWP-File-Path` - URL-encoded relative file path
* `X-MigWP-File-Size` - Full file size in bytes
* `X-MigWP-Chunk-Size` - Fixed chunk size in bytes (`10485760`)
* `X-MigWP-Chunk-Index` - Current chunk index
* `X-MigWP-Chunk-Checksum` - SHA-256 checksum of the returned chunk body
* `X-MigWP-Chunk-Bytes` - Byte range of the returned chunk
* `X-MigWP-Total-Chunks` - Total number of chunks for the file
* `X-MigWP-Is-Last-Chunk` - `1` for the final chunk, otherwise `0`

Resume strategy:

1. Request chunk `0`.
2. Verify the body against `X-MigWP-Chunk-Checksum`.
...
1. **Admin Interface** – Easily configure your migration settings for a seamless transfer to MigWP.
...
### 1.0.0 (16 March, 2025)
- 🎉 **Initial Release** – Migrate WordPress sites to MigWP with ease.

== Upgrade Notice ==

= 1.0.0 =
Welcome to MigWP Migrator! Start enjoying better WordPress hosting today.
